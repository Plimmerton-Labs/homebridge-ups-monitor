 ...
 *   POST /outages       → returns outage timeline events
 *   POST /outages/acknowledge → acknowledges the latest outage
 *   POST /outages/clear → clears outage timeline events
...
class DashboardServer {
    ...
    if (req.method === 'POST') {
      ...
      req.on('end', async () => {
        ...
        try {
          ...
          else if (url === '/outages')      result = await this._handleOutages(body);
          else if (url === '/outages/acknowledge') result = await this._handleOutagesAcknowledge(body);
          else if (url === '/outages/clear') result = await this._handleOutagesClear(body);
          else if (url === '/outages/export') result = await this._handleOutagesExport(body);
  ...
  async _handleHistory(body = {}) {
    ...
    return { success: true, upsName, points: telemetryStore.readHistory(this._storagePath, upsName) };
  }

  async _handleExport(body = {}) {
    const upsName = this._resolveUpsName(body);
    return { success: true, upsName, ...telemetryStore.buildHistoryCsv(this._storagePath, upsName) };
  }

  async _handleExport30d(body = {}) {
    const upsName = this._resolveUpsName(body);
    return { success: true, upsName, ...telemetryStore.build30dCsv(this._storagePath, upsName) };
  }

  async _handleLogs(body = {}) {
    const upsName = this._resolveUpsName(body);
    return { success: true, upsName, files: telemetryStore.listLogs(this._storagePath, upsName) };
  }

  async _handleOutages(body = {}) {
    const upsName = this._resolveUpsName(body);
    return { success: true, upsName, ...telemetryStore.readOutages(this._storagePath, upsName) };
  }

  async _handleOutagesAcknowledge(body = {}) {
    const upsName = this._resolveUpsName(body);
    return { success: true, upsName, ...telemetryStore.acknowledgeLatestOutage(this._storagePath, upsName) };
  }

  async _handleOutagesClear(body = {}) {
    const upsName = this._resolveUpsName(body);
    return { success: true, upsName, ...telemetryStore.clearOutages(this._storagePath, upsName) };
  }

  async _handleOutagesExport(body = {}) {
    const upsName = this._resolveUpsName(body);
    return { success: true, upsName, ...telemetryStore.buildOutageCsv(this._storagePath, upsName) };
  }

  async _handleLogsDownload(body = {}) {
    return telemetryStore.readLogFile(this._storagePath, body.filename);
  }
}
