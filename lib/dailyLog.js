 ...
 *   2024-06-01T00:01:05.123Z,230.5,229.8,87,22,18.50
 *   ...
 *
 * Usage:
 *   const DailyLog = require('./lib/dailyLog');
 *   const log = new DailyLog('/var/lib/homebridge', 'ups', 30);
 *   log.append({ t: new Date().toISOString(), inV: 230.5, outV: 229.8, bat: 87, load: 22, runtime: 1110 });
 *
 * Notes:
...
const CSV_HEADER = 'timestamp,input_voltage,output_voltage,battery_pct,load_pct,runtime_min\n';
...
class DailyLog {
  ...
  constructor(storageDir, upsName, retainDays = 30) {
    ...
    // Note: the data directory is created lazily on first append() so that a
    // freshly-started plugin with no telemetry yet leaves no empty folders.
    this._prune();
  }
   ...
   *
   * `runtime` is given in seconds (as stored on the telemetry point) and written
   * to the `runtime_min` column in minutes, matching the dashboard 24h export.
   *
   * @param {{ t: string, inV: number|null, outV: number|null,
   *            bat: number|null, load: number|null, runtime: number|null }} point
   */
  append({ t, inV, outV, bat, load, runtime }) {
    this._ensureDir();
    const filePath = this._fileForDate(this._dateStr(new Date()));
    const isNew    = !fs.existsSync(filePath);

    const runtimeMin = runtime != null ? (runtime / 60).toFixed(2) : '';
    const line = `${t ?? ''},${inV ?? ''},${outV ?? ''},${bat ?? ''},${load ?? ''},${runtimeMin}\n`;
    try {
      if (isNew) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (!isNaN(fileDate.getTime()) && fileDate < cutoff) {
          try { fs.unlinkSync(path.join(this.storageDir, file)); } catch { /* ignore */ }
        }
      }
    } catch {
      // Best-effort
