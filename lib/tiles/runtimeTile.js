...
module.exports = function setupRuntimeTile(accessory, api, upsName) {
  ...
  // HAP only permits alphanumerics, spaces, and apostrophes in names — the old
  // '(min)' suffix triggered a warning and could break Home-app pairing.
  const NAME = `${upsName} Runtime in Mins`;

  let svc = accessory.getService(Service.TemperatureSensor);
  if (!svc) {
    svc = accessory.addService(Service.TemperatureSensor, NAME);
  } else if (svc.displayName !== NAME) {
    // Repair accessories cached by older versions that used the invalid name.
    svc.displayName = NAME;
    if (Characteristic.Name) svc.updateCharacteristic(Characteristic.Name, NAME);
  }
